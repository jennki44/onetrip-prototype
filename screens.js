...
SCREENS.scan = ({ itemId, fail }) => h`<div class="screen" style="max-width:520px">${backBtn()}
  ...
  <div class="camera"><div class="frame"><i></i></div>
    <div class="paper"><b>SUSHI HOUSE<br>GINZA</b><div class="r"><span>2025/10/14 13:41</span><span>#0412</span></div><div style="border-top:1px dashed #999;margin:4px 0"></div><div class="r"><span>Salmon sushi</span><span>3,000</span></div><div class="r"><span>Ramen</span><span>2,000</span></div><div class="r"><span>Sashimi moriawase</span><span>4,000</span></div><div class="r"><span>Drinks ×6</span><span>3,500</span></div><div class="r"><span>Dessert ×2</span><span>2,500</span></div><div class="r"><span>Edamame &amp; pickles</span><span>1,700</span></div><div class="r tot"><span>Subtotal</span><span>16,700</span></div><div class="r"><span>Tax 10%</span><span>1,700</span></div><div class="r tot"><span>TOTAL</span><span>¥18,400</span></div><div class="r"><span>CARD ****4021</span><span>APPROVED</span></div></div>
...
SCREENS.memories = () => { const places = new Set(S.itinerary.filter(i => i.placeId).map(i => i.placeId)).size; const rest = S.places.filter(p => p.type === 'restaurant').length; const spentAll = spentJpy(); const big = [...S.expenses].sort((a, b) => b.jpy - a.jpy)[0]; const bestRated = [...S.places].filter(p => p.rating).sort((a, b) => b.rating - a.rating)[0];
