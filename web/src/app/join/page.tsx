...
export default async function Join({ searchParams }: { searchParams: Promise<{ code?: string; error?: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[440px] px-6 pt-8">
      ...
      {code.length >= 6 && !preview && <p className="mt-4 text-[14px] text-ink-2">No trip matches that code.</p>}
      {preview && (
        <div className="card mt-4"><div className="flex items-center gap-3"><span className="text-[36px]">{preview.emoji}</span><div><div className="text-[20px] font-bold">{preview.name}</div><div className="text-ink-2">{preview.destination} · {preview.members} {t("common.travellers")} · {preview.plans} plans</div></div></div>
          ...
          {!user && <p className="mt-2 text-[12px] text-ink-3">You&apos;ll sign in with a magic link first, then land straight in the trip.</p>}
        </div>)}
    </div>
  );
}
