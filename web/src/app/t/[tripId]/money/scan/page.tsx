...
export default async function Scan({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ for?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[560px]"><Link href={`${base}/money`} className="mb-2 inline-block text-[0.875rem] font-extrabold text-ink-2">‹ {t("money.title")}</Link><h1 className="text-[1.75rem]">{t("receipt.scan")}</h1><p className="mb-4 text-ink-2">{t("receipt.frame")}</p>
