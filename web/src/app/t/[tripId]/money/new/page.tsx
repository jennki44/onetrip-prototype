...
export default async function NewExpense({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ for?: string; mode?: string }> }) {
  ...
  if (!sp.mode && !item) return (
    <div className="mx-auto max-w-[560px]"><Link href={`${base}/money`} className="mb-2 inline-block text-[0.875rem] font-extrabold text-ink-2">‹ {t("money.title")}</Link><h1 className="text-[1.75rem]">{t("money.addExpense")}</h1><p className="mb-4 text-ink-2">{t("ui.fastest")}</p>
      <div className="flex flex-col gap-3">{[["📷", t("money.scan"), t("ui.scanSub"), `${base}/money/scan`], ["✍️", t("money.manual"), t("ui.manualSub"), `${base}/money/new?mode=manual`]].map(([e, title, sub, href]) => <Link key={href} href={href} className="card flex items-center gap-3.5"><span className="text-[1.875rem]">{e}</span><div className="flex-1"><div className="text-[1rem] font-bold">{title}</div><div className="text-[0.7813rem] text-ink-3">{sub}</div></div><span className="text-ink-3">›</span></Link>)}</div></div>);
  return (
    <div className="mx-auto max-w-[560px]"><Link href={`${base}/money`} className="mb-2 inline-block text-[0.875rem] font-extrabold text-ink-2">‹ {t("money.title")}</Link><h1 className="mb-4 text-[1.75rem]">{item ? `${t("money.addExpense")} · ${item.title}` : t("money.manual")}</h1>
