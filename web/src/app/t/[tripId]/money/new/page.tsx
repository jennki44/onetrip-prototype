...
export default async function NewExpense({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ for?: string; mode?: string }> }) {
  ...
  if (!sp.mode && !item) return (
    <div className="mx-auto max-w-[560px]"><Link href={`${base}/money`} className="mb-2 inline-block text-[14px] font-extrabold text-ink-2">‹ {t("money.title")}</Link><h1 className="text-[28px]">{t("money.addExpense")}</h1><p className="mb-4 text-ink-2">Pick the fastest way. Everything ends up in the same place.</p>
      <div className="flex flex-col gap-3">{[["📷", t("money.scan"), "Take a photo of the receipt and itemise it.", `${base}/money/scan`], ["✍️", t("money.manual"), "Type the amount and choose who was there.", `${base}/money/new?mode=manual`]].map(([e, title, sub, href]) => <Link key={href} href={href} className="card flex items-center gap-3.5"><span className="text-[30px]">{e}</span><div className="flex-1"><div className="text-[16px] font-bold">{title}</div><div className="text-[12.5px] text-ink-3">{sub}</div></div><span className="text-ink-3">›</span></Link>)}</div></div>);
  return (
