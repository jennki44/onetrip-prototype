...
export default async function Settle({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[560px]">
      ...
      {pl.length ? <div className="flex flex-col gap-3">{pl.map((p, k) => <div key={k} className="card"><div className="flex items-center gap-3"><Avatar p={m(p.fromUserId)} size="lg" /><div className="flex-1"><div className="text-[16px] font-bold">{m(p.fromUserId).name} → {m(p.toUserId).name}</div><div className="text-[12.5px] text-ink-3">{p.fromUserId === user?.id ? t("ui.youPay") : p.toUserId === user?.id ? t("ui.paysYou") : t("ui.between")}</div></div><Avatar p={m(p.toUserId)} size="lg" /></div><div className="mt-3"><div className="num font-display text-[30px] font-bold">{inCurrency(b, p.amountMinor, rc)}</div><div className="text-[12.5px] text-ink-3">{fmtBase(b, p.amountMinor)}</div></div><div className="mt-3 flex gap-2"><form action={markPaid} className="flex-1"><input type="hidden" name="tripId" value={tripId} /><input type="hidden" name="index" value={k} /><button className="btn btn-sun w-full">{t("money.markPaid")}</button></form><button className="btn flex-1" disabled>{t("money.remind")}</button></div></div>)}<div className="card flex justify-between bg-surface-2 shadow-none"><span>{t("money.outstanding")}</span><b className="num">{inCurrency(b, total, rc)}</b></div><p className="text-[12.5px] text-ink-3">{t("ui.settleNote")}</p></div>
