...
function Tab({ base, tab, k, label }: { base: string; tab: string; k: string; label: string }) { return <Link href={`${base}/money?tab=${k}`} className={`flex-1 rounded-lg px-3 py-2 text-center text-[0.8438rem] font-extrabold ${tab === k ? "bg-surface shadow-card" : "text-ink-3"}`}>{label}</Link>; }
        ...
        <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
          <div>
            <div className="grid grid-cols-2 gap-2.5 md:grid-cols-4">
              {[[t("money.tripBudget"), bud != null ? inCurrency(b, bud, rc, { decimals: 0 }) : "—", `${b.members.length} ${t("common.travellers")}`], [t("home.spent"), inCurrency(b, spent, rc, { decimals: 0 }), `${fmtBase(b, spent)}${bud ? ` · ${Math.round(spent / bud * 100)}%` : ""}`], [t("home.remaining"), bud != null ? inCurrency(b, bud - spent, rc, { decimals: 0 }) : "—", ""], [t("home.forecast"), inCurrency(b, fc, rc, { decimals: 0 }), bud != null ? (fc > bud ? t("ui.over", { amount: inCurrency(b, fc - bud, rc, { decimals: 0 }) }) : t("ui.under", { amount: inCurrency(b, bud - fc, rc, { decimals: 0 }) })) : ""]].map(([l, v, s], k) => <div key={k} className="card px-4 py-3.5"><div className="eyebrow mb-1">{l}</div><div className={`num font-display text-[1.375rem] font-bold ${k === 2 ? "text-good" : k === 3 && bud != null && fc > bud ? "text-warn" : ""}`}>{v}</div><div className="text-[0.7813rem] text-ink-2">{s}</div></div>)}
            </div>
            {bud != null && fc > bud && <div className="mt-3 flex gap-2 rounded-xl bg-warn-soft p-3 text-[0.875rem]">⚠️<span>{t("home.over", { amount: inCurrency(b, fc - bud, rc, { decimals: 0 }) })}</span></div>}
          ...
          <div className="card mt-3 md:mt-0"><div className="eyebrow mb-3">{t("money.byCategory")}</div>
            {Object.keys(b.trip.budget_categories || {}).map(c => { const v = cats[c] || 0; const cb = catBudgetBase(b, c); const pct = cb ? v / cb * 100 : 0; return <div key={c} className="border-t border-line-2 py-2.5 first:border-t-0"><div className="mb-1.5 flex items-center justify-between text-[0.875rem]"><span>{CAT_EMOJI[c]} {t(`money.categories.${c}`)}</span><span className="num text-[0.7813rem]"><b>{inCurrency(b, v, rc, { decimals: 0 })}</b> / {inCurrency(b, cb, rc, { decimals: 0 })}</span></div><Bar pct={pct} tone={pct > 100 ? "bad" : pct > 80 ? "warn" : undefined} /></div>; })}
          </div>
        </div>
      )}

      {tab === "expenses" && (b.expenses.length ? (() => { const list = [...b.expenses].sort((x, y) => (y.date + (y.time || "")).localeCompare(x.date + (x.time || ""))); const groups = new Map<string, typeof list>(); for (const e of list) { const d = new Date(e.date + "T00:00:00"); const key = d.toLocaleDateString(locale.startsWith("zh") ? "zh-Hant-HK" : "en-AU", { weekday: "long", day: "numeric", month: "short" }); groups.set(key, [...(groups.get(key) || []), e]); } return <div>{[...groups.entries()].map(([k, es]) => <section key={k} className="mb-4"><div className="eyebrow mb-2 flex justify-between"><span>{k}</span><span>{inCurrency(b, es.reduce((a, e) => a + e.base_minor, 0), rc)}</span></div><div className="card divide-y divide-line-2 p-0">{es.map(e => <Link key={e.id} href={`${base}/money/${e.id}`} className="flex items-center gap-3 px-4 py-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-surface-2 text-[1.1875rem]">{e.emoji || CAT_EMOJI[e.category] || "💰"}</span><div className="min-w-0 flex-1"><div className="truncate font-bold">{e.merchant}</div><div className="text-[0.8125rem] text-ink-2">{t("money.paidBy", { name: nameOf(e.payer_id) })} · {b.shares.filter(s => s.expense_id === e.id).length} {t("common.people")}{e.receipt_id ? " · 🧾" : ""}</div></div><div className="text-right"><div className="num font-display font-bold">{inCurrency(b, e.amount_minor, e.currency)}</div>{e.currency !== rc && <div className="text-[0.75rem] text-ink-3">≈ {inCurrency(b, e.base_minor, rc)}</div>}</div></Link>)}</div></section>)}</div>; })() : <Empty emoji="💵" title={t("money.empty")} sub={t("money.emptySub")} action={<Link href={`${base}/money/new`} className="btn btn-sun">{t("money.addExpense")}</Link>} />)}

      {tab === "balances" && (() => { const net = netBalances(b); const pl = plan(b); const outstanding = pl.reduce((a, p) => a + p.amountMinor, 0); return (
        <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
          <div><div className="eyebrow mb-2">{t("money.whoOwes")}</div><div className="card divide-y divide-line-2 p-0">{b.members.map(m => { const v = net[m.user_id] || 0; const settled = Math.abs(v) < 1; return <div key={m.user_id} className="flex items-center gap-3 px-4 py-3"><Avatar p={m.profile} /><div className="flex-1"><div className="font-bold">{m.profile.name}{m.user_id === user?.id ? ` ${t("ui.you")}` : ""}</div><div className="text-[0.8125rem] text-ink-2">{settled ? t("money.settled") : v > 0 ? t("money.isOwed") : t("money.owes")}</div></div><div className={`num font-display text-[1.0625rem] font-bold ${settled ? "text-ink-3" : v > 0 ? "text-good" : "text-bad"}`}>{settled ? "✓" : inCurrency(b, Math.abs(v), rc)}</div></div>; })}</div><p className="mt-2 text-[0.7813rem] text-ink-3">{t("ui.balancesNote", { n: b.settlements.length })}</p></div>
