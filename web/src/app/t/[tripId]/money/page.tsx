...
import { budgetBase, catBudgetBase, dayCount, dayDate, forecastBase, inCurrency, netBalances, nowSlots, plan, spentBase, spentByCategory, fmtBase } from "@/lib/trip/derive";
  ...
  return (
    <div>
      ...
      {tab === "overview" && (
        <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
          <div>
            ...
            <div className="card mt-3"><div className="eyebrow mb-2">{t("money.byDay")}</div>{(() => { const n = dayCount(b.trip); const byDay = Array.from({ length: n }, (_, i) => { const ds = dayDate(b.trip, i + 1).toISOString().slice(0, 10); return b.expenses.filter(e => e.date === ds).reduce((a, e) => a + e.base_minor, 0); }); const mx = Math.max(...byDay, 1); return <svg viewBox={`0 0 ${n * 33} 56`} className="h-14 w-full" preserveAspectRatio="none">{byDay.map((v, i) => <rect key={i} x={i * 33 + 4} y={56 - v / mx * 50} width="26" height={v / mx * 50} rx="4" fill={i + 1 === day ? "var(--coral)" : i + 1 < day ? "var(--teal)" : "var(--line)"} />)}</svg>; })()}</div>
          </div>
      ...
      {tab === "balances" && (() => { const net = netBalances(b); const pl = plan(b); const outstanding = pl.reduce((a, p) => a + p.amountMinor, 0); return (
        <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
          <div><div className="eyebrow mb-2">{t("money.whoOwes")}</div><div className="card divide-y divide-line-2 p-0">{b.members.map(m => { const v = net[m.user_id] || 0; const settled = Math.abs(v) < 1; return <div key={m.user_id} className="flex items-center gap-3 px-4 py-3"><Avatar p={m.profile} /><div className="flex-1"><div className="font-bold">{m.profile.name}{m.user_id === user?.id ? " (you)" : ""}</div><div className="text-[13px] text-ink-2">{settled ? t("money.settled") : v > 0 ? t("money.isOwed") : t("money.owes")}</div></div><div className={`num font-display text-[17px] font-bold ${settled ? "text-ink-3" : v > 0 ? "text-good" : "text-bad"}`}>{settled ? "✓" : inCurrency(b, Math.abs(v), rc)}</div></div>; })}</div><p className="mt-2 text-[12.5px] text-ink-3">Balances come from every expense&apos;s split plus {b.settlements.length} payments already made between travellers.</p></div>
          <div><div className="card mt-4 md:mt-0"><div className="eyebrow mb-2">{t("money.settleUp")}</div>{pl.length ? <><div className="text-[16px] font-bold">{t("money.payments", { n: pl.length })}</div><p className="text-[12.5px] text-ink-2">{t("money.outstanding")}: <b>{inCurrency(b, outstanding, rc)}</b></p><Link href={`${base}/money/settle`} className="btn btn-sun mt-3 w-full">{t("money.simplify")}</Link></> : <div className="text-[16px] font-bold">Everyone is settled up 🎉</div>}</div>
            <div className="card mt-3"><div className="eyebrow mb-2">Payments made</div>{b.settlements.length ? b.settlements.map(s => <div key={s.id} className="flex justify-between border-t border-line-2 py-1.5 text-[12.5px] first:border-t-0"><span>{nameOf(s.from_user)} → {nameOf(s.to_user)}</span><span className="num"><b>{fmtBase(b, s.amount_minor)}</b> · {new Date(s.date + "T00:00:00").toLocaleDateString("en-AU", { day: "numeric", month: "short" })}</span></div>) : <p className="text-[12.5px] text-ink-3">None yet.</p>}</div></div>
        </div>); })()}
    </div>
  );
}
