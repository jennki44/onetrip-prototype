...
export default async function Bookings({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[640px]">
      ...
      <section className="mt-6"><div className="eyebrow mb-2">{t("ui.stillToBook")}</div><div className="card divide-y divide-line-2 p-0">{needed.length ? needed.map(i => <Link key={i.id} href={`${base}/plan/${i.id}`} className="flex items-center gap-3 px-4 py-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-surface-2 text-[19px]">{i.emoji}</span><div className="flex-1"><div className="font-bold">{i.title}</div><div className="text-[13px] text-ink-2">{dayLabel(b.trip, i.day, locale)} · {fmtTime(i.start_time, locale)}</div></div><span className="btn btn-sun btn-sm">{t("item.resolve")}</span></Link>) : <div className="flex items-center gap-3 px-4 py-3"><span className="text-[19px]">✅</span><b>{t("ui.allBooked")}</b></div>}</div></section>
    </div>
  );
}
