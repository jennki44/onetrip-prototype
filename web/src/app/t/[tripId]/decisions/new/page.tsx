...
export default async function NewDecision({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ place?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[560px]"><Link href={`${base}/decisions`} className="mb-2 inline-block text-[0.875rem] font-extrabold text-ink-2">‹ {t("decisions.title")}</Link><h1 className="text-[1.75rem]">{t("ui.startDecision")}</h1><p className="mb-4 text-ink-2">{t("ui.startDecisionSub")}</p>
      <form action={createDecision} className="flex flex-col gap-3"><input type="hidden" name="tripId" value={tripId} />
        ...
        <div><div className="eyebrow mb-2">{t("ui.options")}</div><div className="grid grid-cols-2 gap-1.5">{b.places.filter(p => p.type === "restaurant" || p.type === "activity" || p.type === "shopping").map(p => <label key={p.id} className="flex items-center gap-2 rounded-xl border-2 border-line bg-surface px-2.5 py-2 text-[0.8125rem] font-bold has-[:checked]:border-teal has-[:checked]:bg-teal-soft"><input type="checkbox" name="places" value={p.id} defaultChecked={p.id === pl?.id} className="hidden" /><span>{p.emoji}</span><span className="truncate">{(locale.startsWith("zh") && p.name_zh) || p.name}</span></label>)}</div></div>
        <button className="btn btn-sun w-full py-4 text-[1rem]">{t("ui.createDecision")}</button>
      </form>
    </div>
  );
}
