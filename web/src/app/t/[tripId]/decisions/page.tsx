...
export default async function Decisions({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  return (
    <div>
      ...
      {done.length > 0 && <section className="mt-6"><div className="eyebrow mb-2">{t("decisions.decided")}</div><div className="card divide-y divide-line-2 p-0">{done.map(d => <Link key={d.id} href={`${base}/decisions/${d.id}`} className="flex items-center gap-3 px-4 py-3"><span className="text-[19px]">✅</span><div className="flex-1"><b>{decTitle(d, locale)}</b><div className="text-[12.5px] text-ink-3">{d.confirmed_option_id ? name(d.confirmed_option_id) : ""}{d.day ? ` · ${dayLabel(b.trip, d.day, locale)}${d.slot ? ` ${fmtTime(d.slot.slice(0, 5), locale)}` : ""}` : ""}</div></div><span className="text-ink-3">›</span></Link>)}</div></section>}
    </div>
  );
}
