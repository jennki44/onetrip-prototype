...
export default async function DecisionDetail({ params }: { params: Promise<{ tripId: string; decisionId: string }> }) {
  ...
  return (
    <div>
      <Link href={`${base}/decisions`} className="mb-2 inline-block text-[0.875rem] font-extrabold text-ink-2">‹ {t("decisions.title")}</Link>
      <div className="mb-3 flex items-start justify-between gap-3"><div><h1 className="text-[1.75rem] leading-tight">{decTitle(d, locale)}</h1><p className="text-[0.9063rem] text-ink-2">{decQuestion(d, locale)}</p></div><Pill tone={confirmed ? "good" : "warn"}>{confirmed ? t("decisions.confirmed") : voters.size >= b.members.length - 1 ? t("decisions.almost") : t("decisions.needsVotes")}</Pill></div>
      <div className="mb-4 flex items-center gap-2 text-[0.7813rem]"><div className="flex-1"><Bar pct={voters.size / b.members.length * 100} tone={confirmed ? "good" : undefined} /></div><b className="num">{voters.size} / {b.members.length}</b><span className="text-ink-3">· {voters.size === b.members.length ? t("decisions.everyoneVoted") : `${b.members.filter(m => !voters.has(m.user_id)).map(m => m.profile.name).join(", ")} ${t("decisions.stillToVote")}`}</span></div>
      {confirmed && d.confirmed_option_id && <div className="mb-4 flex gap-2 rounded-xl bg-good-soft p-3 text-[0.875rem]">✅<span><b>{name(opts.find(o => o.id === d.confirmed_option_id)!)}</b> {t("decisions.confirmed").toLowerCase()}. {d.day ? t("decisions.addedTo", { day: dayLabel(b.trip, d.day, locale), time: d.slot ? fmtTime(d.slot.slice(0, 5), locale) : "" }) : ""} <Link href={`${base}/plan?day=${d.day}`} className="font-extrabold text-teal-text">{t("plan.title")} ›</Link></span></div>}
        ...
        <div className="flex flex-col gap-3">
          {opts.map(o => { const p = place(b, o.place_id); const votes = b.votes.filter(v => v.option_id === o.id); const mine = votes.find(v => v.user_id === user?.id)?.reaction; const isLead = o.id === lead?.id && !confirmed; const isConf = confirmed && d.confirmed_option_id === o.id; return (
            <div key={o.id} className={`card ${isConf ? "bg-teal-soft" : ""} ${isLead ? "outline outline-2 outline-teal" : ""}`}>
              <div className="flex items-start justify-between gap-2"><div className="flex gap-3">{p?.photo_url ? <img src={p.photo_url} alt="" className="h-16 w-16 rounded-2xl object-cover" /> : <span className="text-[1.75rem]">{p?.emoji || "🗳"}</span>}<div><div className="text-[1rem] font-bold">{name(o)}</div><div className="text-[0.7813rem] text-ink-3">{p ? `${p.price_level || ""}${p.rating ? ` · ${p.rating} ⭐` : ""}${p.from_hotel_min != null ? ` · ${p.from_hotel_min} min` : ""}` : optionSub(o, locale)}</div><div className="text-[0.7813rem] text-ink-3">{t("decisions.estimated")} <b>{inCurrency(b, o.est_pp_minor, rc)}{t("decisions.perPerson")}</b></div></div></div>{isConf ? <Pill tone="good">{t("decisions.confirmed")}</Pill> : isLead ? <Pill tone="teal">{t("home.leading")}</Pill> : null}</div>
              <div className="mt-3 flex items-center gap-2 text-[0.8125rem] font-bold text-ink-2">{(["love", "good", "maybe", "no"] as const).map(r => <span key={r}>{REACT_EMOJI[r]} {votes.filter(v => v.reaction === r).length}</span>)}<span className="ml-auto"><Avatars people={votes.map(v => b.members.find(m => m.user_id === v.user_id)?.profile).filter(Boolean) as { name: string; initials: string; color: string }[]} /></span></div>
              {!confirmed && <form action={castVote} className="mt-3 grid grid-cols-4 gap-1.5"><input type="hidden" name="tripId" value={tripId} /><input type="hidden" name="decisionId" value={d.id} /><input type="hidden" name="optionId" value={o.id} />{(["love", "good", "maybe", "no"] as const).map(r => <button key={r} name="reaction" value={r} className={`flex flex-col items-center gap-0.5 rounded-xl border-2 py-2 text-[1.125rem] ${mine === r ? "border-teal bg-teal-soft" : "border-transparent bg-surface-2"}`}>{REACT_EMOJI[r]}<small className="text-[0.6563rem] font-extrabold text-ink-2">{t(`decisions.${r}`)}</small></button>)}</form>}
            </div>); })}
        ...
        <div>
          {!confirmed && lead && (
            <div className="mt-4 rounded-[20px] border border-line bg-gradient-to-br from-teal-soft to-surface p-4 md:mt-0">
              ...
              <div className="mb-2 text-[1rem] font-bold">{t("decisions.strongest", { name: name(lead) })}</div>
              <div className="text-[0.7813rem] font-bold text-ink-2">{t("decisions.why")}</div>
          ...
          {today > 0 && d.day && d.day < today && !confirmed && <p className="mt-3 text-[0.7813rem] text-warn">{t("ui.dayPassed")}</p>}
        </div>
      </div>
    </div>
  );
