...
export default async function Travellers({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[640px]">
      <PageHead title={t("more.travellers")} sub="Everyone sees the same trip." right={me && ["owner", "admin"].includes(me.role) ? <Link href={`${base}/invite`} className="btn btn-sun btn-sm">+ Invite</Link> : undefined} />
      <div className="card divide-y divide-line-2 p-0">{b.members.map(m => <div key={m.user_id} className="flex items-center gap-3 px-4 py-3"><Avatar p={m.profile} size="lg" /><div className="flex-1"><div className="font-bold">{m.profile.name}{m.user_id === user?.id ? " (you)" : ""}</div><div className="text-[13px] capitalize text-ink-2">{m.role} · {t("invite.joined")}</div></div><Pill tone={m.role === "owner" ? "teal" : undefined}>{m.role}</Pill></div>)}</div>
