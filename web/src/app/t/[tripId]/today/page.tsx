  ...
  const block = (label: string, i: typeof current, extra?: string) => i ? (
    <section className="mt-5"><Eyebrow>{label}</Eyebrow>
      <Link href={`${base}/plan/${i.id}`} className="card flex items-center gap-3.5">
        ...
        <div className="min-w-0 flex-1"><div className="font-display text-[20px] font-bold leading-tight">{itemTitle(i, locale)}</div><div className="text-[14px] text-ink-2">{fmtTime(i.start_time, locale)}{place(b, i.place_id) ? ` · ${placeName(place(b, i.place_id), locale)}` : ""}</div>{extra && <div className="mt-1 text-[12.5px] text-ink-3">{extra}</div>}</div><span className="text-ink-3">›</span>
      </Link></section>) : null;
  return (
    <div>
      ...
      <div className="md:grid md:grid-cols-2 md:gap-6">
        <div>
          ...
          {later.length > 0 && <section className="mt-5"><Eyebrow>{t("today.later")}</Eyebrow><div className="card divide-y divide-line-2 p-0">{later.map(i => <Link key={i.id} href={`${base}/plan/${i.id}`} className="flex items-center gap-3 px-4 py-3">{photoOf(b, i) ? <img src={photoOf(b, i)!} alt="" className="thumb h-10 w-10 rounded-xl" /> : <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-surface-2 text-[19px]">{i.emoji}</span>}<div className="min-w-0 flex-1"><div className="font-bold">{itemTitle(i, locale)}</div><div className="text-[13px] text-ink-2">{fmtTime(i.start_time, locale)}{place(b, i.place_id) ? ` · ${placeName(place(b, i.place_id), locale)}` : ""}</div></div>{i.cost_minor ? <span className="text-[12.5px] text-ink-3">{inCurrency(b, i.cost_minor, rc)}</span> : null}</Link>)}</div></section>}
        </div>
        <div>
          ...
          {openForMe.length > 0 && <section className="mt-5"><Eyebrow>{t("today.action")}</Eyebrow>{openForMe.map(d => <Link key={d.id} href={`${base}/decisions/${d.id}`} className="card mb-2 flex items-center gap-3"><span className="text-[24px]">🗳</span><div className="flex-1"><b>{t("today.vote", { title: decTitle(d, locale) })}</b><div className="text-[12.5px] text-ink-3">{votersOf(b, d).size} / {b.members.length}</div></div><span className="text-ink-3">›</span></Link>)}</section>}
        </div>
      </div>
    </div>
  );
