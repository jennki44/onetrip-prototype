...
export default async function Brain({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[680px]">
      ...
      {a && <div className="mb-4 flex flex-col gap-3"><div className="self-end rounded-2xl rounded-br-md bg-teal px-3.5 py-3 text-[0.9063rem] text-white">{q}</div><div className="max-w-[92%] rounded-2xl rounded-bl-md bg-surface px-3.5 py-3 text-[0.9063rem] shadow-card"><div dangerouslySetInnerHTML={{ __html: a.html }} />{a.note && <div className="mt-2 text-[0.7188rem] text-ink-3">{a.note}</div>}{a.actions?.length ? <div className="mt-2 flex flex-wrap gap-1.5">{a.actions.map(x => <Link key={x.href} href={x.href} className="btn btn-sun btn-sm">{x.label}</Link>)}</div> : null}</div></div>}
      <div className="eyebrow mb-2">{a ? t("brain.another") : t("brain.try")}</div>
      <div className="mb-3 flex flex-wrap gap-2">{SUGGESTIONS.filter(s => s !== q).slice(0, a ? 4 : 8).map(s => <Link key={s} href={`${base}/brain?q=${encodeURIComponent(s)}`} className="rounded-full border border-line bg-surface px-3 py-2 text-[0.8125rem] font-semibold">{s}</Link>)}</div>
      ...
      <p className="mt-3 text-[0.7813rem] text-ink-3">{t("brain.note")}</p>
    </div>
  );
}
