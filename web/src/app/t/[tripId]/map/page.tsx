...
import { itemTitle, itemsOnDay, nowSlots } from "@/lib/trip/derive";
...
export default async function MapPage({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ focus?: string; filter?: string }> }) {
  const { tripId } = await params; const sp = await searchParams; const [b, { t, locale }] = await Promise.all([loadTrip(tripId), getT()]); if (!b) notFound();
