...
export default async function Health({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[640px]">
      ...
      <div className="card flex items-center gap-3.5"><span className="h-[18px] w-[18px] rounded-full" style={{ background: `var(--${ov})` }} /><div><div className="eyebrow">{t("health.overall")}</div><div className="text-[1.25rem] font-bold">{label}</div></div><span className="ml-auto text-[0.7813rem] text-ink-3">{checks.filter(c => c.tone === "good").length} ✓ · {checks.filter(c => c.tone === "warn").length} ⚠ · {checks.filter(c => c.tone === "bad").length} !</span></div>
      <div className="card mt-3 divide-y divide-line-2 p-0">
        {checks.map((c, k) => <Link key={k} href={c.link} className="flex items-center gap-3 px-4 py-3"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: `var(--${c.tone})` }} /><div className="min-w-0 flex-1"><div className="font-semibold">{c.text}</div>{c.sub && <div className="text-[0.7813rem] text-ink-3">{c.sub}</div>}</div>{c.resolve ? <span className="btn btn-sun btn-sm">{t("health.resolve")}</span> : <span className="text-ink-3">›</span>}</Link>)}
      </div>
    </div>
  );
}
