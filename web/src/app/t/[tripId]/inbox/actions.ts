...
import { dayDate, isoDate } from "@/lib/trip/derive";
...
export async function registerDocument(raw: z.input<typeof doc> & { linkedBookingId: string | null }) {
  ...
  if (p.linkedBookingId) { const { data: bk } = await sb.from("bookings").select("id").eq("id", p.linkedBookingId).eq("trip_id", p.tripId).maybeSingle(); if (!bk) throw new Error("That booking is not in this trip"); }
  if (p.storagePath && !p.storagePath.startsWith(`${p.tripId}/`)) throw new Error("Bad storage path");
...
export async function addBookingFromInbox(raw: z.input<typeof doc> & { title: string; provider: string; reference: string; cost: string; itemId: string | null }) {
  ...
  const { data: bk } = await sb.from("bookings").insert({ trip_id: p.tripId, type, title: p.title, provider: p.provider || null, reference: p.reference || null, date: item ? isoDate(dayDate(b.trip, item.day)) : null, status: "confirmed", cost_minor: costMinor, details: {}, created_by: user.id }).select("id").single();
