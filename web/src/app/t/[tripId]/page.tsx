      ...
      <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
        <div>
          <div className="card-hero">
            <div className="flex items-center justify-between"><span className="eyebrow text-white/85">{before ? t("ui.dayN", { n: 1 }) : t("home.next")}</span><span className="text-[0.75rem] text-white/85">{!before && !after ? dayLabel(b.trip, day, locale) : ""}</span></div>
            {(() => { const n = before ? (b.items.filter(i => i.day === 1).sort((x, y) => x.start_time.localeCompare(y.start_time))[0] || null) : next; if (!n) return <p className="mt-2 text-white/90">{t("home.nothingElse")}</p>; const ph = photoOf(b, n); const pl = place(b, n.place_id); return (
              <Link href={`${base}/plan/${n.id}`} className="mt-2 flex items-center gap-3.5">
                {ph ? <img src={ph} alt="" className="h-16 w-16 rounded-2xl object-cover ring-2 ring-white/40" /> : <span className="text-[2.5rem]">{n.emoji}</span>}
                <div className="min-w-0 flex-1"><div className="font-display text-[1.25rem] font-bold leading-tight">{itemTitle(n, locale)}</div><div className="text-[0.875rem] text-white/90">{fmtTime(n.start_time, locale)}{pl ? ` · ${placeName(pl, locale)}` : ""}</div>{n.travel_min ? <div className="mt-1 text-[0.75rem] text-white/80">🚗 {t("home.minAway", { n: n.travel_min })}{current ? ` · ${t("today.now").toLowerCase()}: ${itemTitle(current, locale)}` : ""}</div> : null}</div>
            ...
            <div className="card divide-y divide-line-2 p-0">
              {(before ? b.items.filter(i => i.day === 1).sort((x, y) => x.start_time.localeCompare(y.start_time)) : items).map(i => (
                <Link key={i.id} href={`${base}/plan/${i.id}`} className="flex items-center gap-3 px-4 py-3">
                  <span className="num w-[66px] shrink-0 text-[0.8125rem] font-bold text-ink-2">{fmtTime(i.start_time, locale)}</span><span>{i.emoji}</span>
          ...
          <section className="mt-5">
            ...
            {open.slice(0, 2).map(d => { const n = votersOf(b, d).size; const lead = leadingOption(b, d); const mine = b.votes.some(v => v.decision_id === d.id && v.user_id === user?.id); const leadName = lead ? optionLabel(b, lead, locale) : null; return (
              <Link key={d.id} href={`${base}/decisions/${d.id}`} className="card mb-3 block">
                <div className="flex items-start justify-between gap-2"><div><div className="text-[1rem] font-bold">{decTitle(d, locale)}</div><div className="text-[0.7813rem] text-ink-3">{decQuestion(d, locale)}</div></div><Pill tone="warn">{n >= b.members.length - 1 ? t("decisions.almost") : t("decisions.needsVotes")}</Pill></div>
                <div className="mt-3 flex items-center gap-2"><div className="flex-1"><Bar pct={n / b.members.length * 100} /></div><span className="num text-[0.7813rem] font-bold">{t("home.voted", { n, total: b.members.length })}</span></div>
                <div className="mt-3 flex items-center justify-between text-[0.7813rem]"><span>{leadName ? <>{t("home.leading")}: <b>{leadName}</b>{lead ? ` · ≈ ${inCurrency(b, lead.est_pp_minor, rc)}${t("decisions.perPerson")}` : ""}</> : ""}</span>{!mine && <Pill tone="sun">{t("home.needsVote")}</Pill>}</div>
              </Link>); })}
        ...
        <div>
          ...
          <section className="mt-5">
            ...
            <div className="card divide-y divide-line-2 p-0">
              {checks.slice(0, 5).map((c, k) => <Link key={k} href={c.link} className="flex items-center gap-3 px-4 py-3"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: `var(--${c.tone})` }} /><span className="min-w-0 flex-1 truncate text-[0.9063rem] font-semibold">{c.text}</span><span className="text-ink-3">›</span></Link>)}
            </div>
          </section>
          {b.notes.length > 0 && <section className="mt-5"><Eyebrow>{t("ui.notes")}</Eyebrow><div className="card divide-y divide-line-2 p-0">{b.notes.map(n => <div key={n.id} className="px-4 py-3 text-[0.9063rem]">📝 {n.text}</div>)}</div></section>}
        </div>
      </div>
    </div>
  );
