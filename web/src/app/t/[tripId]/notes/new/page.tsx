...
export default async function NewNote({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[560px]"><Link href={base} className="mb-2 inline-block text-[14px] font-extrabold text-ink-2">‹ {t("common.back")}</Link><h1 className="text-[28px]">{t("ui.addNote")}</h1><p className="mb-4 text-ink-2">{t("ui.addNoteSub")}</p>
