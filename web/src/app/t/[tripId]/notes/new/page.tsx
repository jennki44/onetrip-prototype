...
export default async function NewNote({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[560px]"><Link href={base} className="btn btn-sm mb-3">‹ {t("common.back")}</Link><h1 className="text-[1.75rem]">{t("ui.addNote")}</h1><p className="mb-4 text-ink-2">{t("ui.addNoteSub")}</p>
