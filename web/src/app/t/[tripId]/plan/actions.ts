...
export async function saveActivity(form: FormData) {
  const p = z.object({ tripId: z.string().uuid(), itemId: z.string().uuid().optional(), title: z.string().trim().min(1).max(120), day: z.coerce.number().int().min(1).max(60), start: time, end: z.union([time, z.literal("")]).optional(), placeId: z.union([z.string().uuid(), z.literal("")]), people: z.array(z.string().uuid()), cost: z.string().optional(), status: z.enum(["idea", "proposed", "voting", "confirmed", "cancelled", "completed"]), note: z.string().max(500).optional() })
    .parse({ tripId: form.get("tripId"), itemId: form.get("itemId") || undefined, title: form.get("title"), day: form.get("day"), start: form.get("start"), end: form.get("end") || "", placeId: form.get("placeId") || "", people: form.getAll("people"), cost: form.get("cost") || "", status: form.get("status"), note: form.get("note") || "" });
  const sb = await supabaseServer(); const { data: { user } } = await sb.auth.getUser(); if (!user) redirect("/signin");
  const b = await loadTrip(p.tripId); if (!b) redirect("/trips"); const pl = p.placeId ? place(b, p.placeId) : null;
