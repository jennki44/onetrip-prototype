  ...
  return (
    <div>
      ...
      <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
        <div>
          {pl && <Link href={`${base}/map?focus=${pl.id}`} className="card flex items-center justify-between"><div><div className="eyebrow mb-1">{t("ui.place")}</div><b>{pl.emoji} {pl.name}</b><div className="text-[12.5px] text-ink-3">{pl.area}{pl.rating ? ` · ⭐ ${pl.rating}` : ""}{pl.price_level ? ` · ${pl.price_level}` : ""}{pl.from_hotel_min != null ? ` · ${pl.from_hotel_min} min` : ""}</div></div><span className="text-ink-3">›</span></Link>}
