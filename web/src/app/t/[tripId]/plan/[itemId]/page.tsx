...
export default async function ItemDetail({ params }: { params: Promise<{ tripId: string; itemId: string }> }) {
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-start gap-3.5"><span className="text-[44px]">{i.emoji}</span><div className="flex-1"><h1 className="text-[28px] leading-tight">{itemTitle(i, locale)}</h1><p className="text-[14.5px] text-ink-2">{dayLabel(b.trip, i.day, locale, true)} · {fmtTime(i.start_time, locale)}{i.end_time && i.end_time !== i.start_time ? ` – ${fmtTime(i.end_time, locale)}` : ""}</p><div className="mt-2 flex flex-wrap gap-1.5"><Pill tone={tone}>{t(`status.${i.status}`)}</Pill>{i.booking === "booked" && <Pill tone="good">🎟 {t("status.booked")}</Pill>}{i.booking === "needed" && <Pill tone="bad">{t("status.notBooked")}</Pill>}{i.cost_minor ? <Pill>{t("ui.est")} {inCurrency(b, i.cost_minor, rc)}</Pill> : null}</div></div><Link href={`${base}/plan/${i.id}/edit`} className="btn btn-outline btn-sm">{t("common.edit")}</Link></div>
      <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
        <div>
          {pl && <Link href={`${base}/map?focus=${pl.id}`} className="card flex items-center justify-between"><div><div className="eyebrow mb-1">{t("ui.place")}</div><b>{pl.emoji} {placeName(pl, locale)}</b><div className="text-[12.5px] text-ink-3">{pl.area}{pl.rating ? ` · ⭐ ${pl.rating}` : ""}{pl.price_level ? ` · ${pl.price_level}` : ""}{pl.from_hotel_min != null ? ` · ${pl.from_hotel_min} min` : ""}</div></div><span className="text-ink-3">›</span></Link>}
          <div className="card mt-3"><div className="eyebrow mb-2">{t("item.who")}</div><div className="flex flex-wrap gap-1.5">{b.members.filter(m => i.participant_ids.includes(m.user_id)).map(m => <span key={m.user_id} className="pill pl-0.5"><Avatar p={m.profile} size="sm" /> {m.profile.name}</span>)}</div>
            ...
            {itemNote(i, locale) && <div className="mt-2 text-[13px] text-ink-2">📝 {itemNote(i, locale)}</div>}
            {itemFlag(i, locale) && <div className="mt-3 flex gap-2 rounded-xl bg-warn-soft p-3 text-[13px]">📌<span>{itemFlag(i, locale)}</span></div>}</div>
        </div>
        <div>
          <div className="card mt-3 md:mt-0"><div className="eyebrow mb-2">{t("item.connected")}</div>
            <div className="divide-y divide-line-2">
              ...
              {dec && <Link href={`${base}/decisions/${dec.id}`} className="flex items-center gap-3 py-3"><span className="text-[19px]">🗳</span><div className="flex-1"><b>{decTitle(dec, locale)}</b><div className="text-[12.5px] text-ink-3">{t(`decisions.${dec.status === "confirmed" ? "confirmed" : "needsVotes"}`)}</div></div><span className="text-ink-3">›</span></Link>}
