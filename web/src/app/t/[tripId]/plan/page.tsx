...
export default async function Plan({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ day?: string; view?: string }> }) {
  ...
  const myRole = b.members.find(m => m.user_id === user?.id)?.role || ""; const canManage = ["owner", "admin"].includes(myRole); const canEdit = ["owner", "admin", "traveller"].includes(myRole);
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {Array.from({ length: total }, (_, i) => i + 1).map(d => { const it = itemsOnDay(b, d); const di = dayL(b.days.find(x => x.day === d), locale); return (
            <Link key={d} href={`${base}/plan?day=${d}`} className={`card min-h-[120px] p-3 ${d === todayDay ? "outline outline-2 outline-coral" : ""}`}>
              <div className="font-display text-[0.9375rem] font-bold">{t("ui.dayN", { n: d })}</div><div className="mb-2 text-[0.7188rem] text-ink-3">{dayLabel(b.trip, d, locale)}{di?.theme ? ` · ${di.theme}` : ""}</div>
              {it.slice(0, 4).map(x => <div key={x.id} className="truncate text-[0.75rem] text-ink-2">{x.emoji} {itemTitle(x, locale)}</div>)}{it.length > 4 && <div className="text-[0.75rem] text-ink-3">+{it.length - 4}</div>}
        ...
        <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
          <div>
            <div className="-mx-4 flex gap-1.5 overflow-x-auto px-4 pb-1 [scrollbar-width:none]">
              {Array.from({ length: total }, (_, i) => i + 1).map(d => { const dd = dayDate(b.trip, d); return <Link key={d} href={`${base}/plan?day=${d}`} className={`w-[54px] shrink-0 rounded-2xl border-2 px-1 py-2 text-center ${d === day ? "border-ink bg-ink text-ground" : d === todayDay ? "border-coral bg-surface" : "border-line bg-surface"}`}><div className="text-[0.6875rem] font-extrabold uppercase opacity-70">{dd.toLocaleDateString(locale.startsWith("zh") ? "zh-Hant-HK" : "en-AU", { weekday: "short" })}</div><div className="font-display text-[1.1875rem] font-bold">{dd.getDate()}</div><div className="text-[0.6563rem] opacity-70">{itemsOnDay(b, d).length}</div></Link>; })}
            </div>
            {info?.banner_url && <div className="relative mt-3 overflow-hidden rounded-[28px]"><img src={info.banner_url} alt="" className="h-[170px] w-full object-cover" />{canEdit && <Link href={`${base}/plan/day?day=${day}`} aria-label={t("dayEdit.edit")} className="absolute right-3 top-3 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-[rgba(255,255,255,.9)] text-[1.1rem] shadow-card">✏️</Link>}<div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[rgba(10,16,20,.82)] to-transparent px-4 pb-3.5 pt-10 text-white"><div className="eyebrow text-white/85">{t("ui.dayN", { n: day })}{info.theme ? ` · ${info.theme}` : ""}</div><div className="font-display text-[1.125rem] font-bold">{dayLabel(b.trip, day, locale, true)}</div><div className="text-[0.7813rem] text-white/85">{info.stay ? `${t("plan.stay")} ${info.stay}` : ""}{info.drive ? ` · ${t("plan.driving")} ${info.drive}` : ""}</div></div></div>}
            {!info?.banner_url && (canEdit ? <Link href={`${base}/plan/day?day=${day}`} className="mt-3 flex items-center gap-3 rounded-[22px] border-2 border-dashed border-teal bg-teal-soft/40 px-4 py-3.5 font-extrabold text-teal-text"><span className="text-[1.6rem]">🖼️</span><span className="flex-1"><span className="block">{t("dayEdit.add")}</span><span className="block text-[0.8125rem] font-semibold text-ink-2">{dayLabel(b.trip, day, locale, true)}{info?.theme ? ` · ${info.theme}` : ""}</span></span><span>›</span></Link> : <div className="mt-3 font-display text-[1.125rem] font-bold">{dayLabel(b.trip, day, locale, true)}{info?.theme ? ` · ${info.theme}` : ""}</div>)}
            {info?.rule && <div className="mt-3 flex gap-2 rounded-xl bg-warn-soft p-3 text-[0.875rem]">📌<span>{info.rule}</span></div>}
            <div className="my-3 flex items-center justify-between"><span className="text-[0.7813rem] text-ink-3">{t("plan.plans", { n: items.length })} · {inCurrency(b, items.reduce((a, i) => a + (i.cost_minor || 0), 0), rc, { decimals: 0 })}{conflicts.size ? ` · ${conflicts.size} ⚠️` : ""}</span><Link href={`${base}/plan/new?day=${day}`} className="btn btn-sun btn-sm">{t("plan.addActivity")}</Link></div>
            {items.length ? (
              <div className="relative pl-[62px] before:absolute before:bottom-2 before:left-[52px] before:top-2 before:w-0.5 before:bg-line">
                {items.map(i => { const done = day < todayDay || (day === todayDay && minutes(i.end_time || i.start_time) <= nm); const isNow = day === todayDay && minutes(i.start_time) <= nm && minutes(i.end_time || i.start_time) > nm; const pl = place(b, i.place_id); const ph = photoOf(b, i); return (
                  <div key={i.id} className={`relative mb-3 ${done ? "opacity-70" : ""}`}>
                    <div className="num absolute -left-[62px] top-3 w-11 text-right text-[0.7813rem] font-bold text-ink-2">{fmtTime(i.start_time, locale).replace(" ", "\n")}</div>
                    ...
                    {i.travel_min ? <div className={`-mt-1 mb-2 ml-1 text-[0.75rem] ${conflicts.has(i.id) ? "font-bold text-bad" : "text-ink-3"}`}>{conflicts.has(i.id) ? "⚠️" : "↓"} {i.travel_min >= 60 ? t("plan.drive", { t: t("plan.hrs", { h: Math.floor(i.travel_min / 60), m: i.travel_min % 60 ? String(i.travel_min % 60) : "" }).trim() }) : t("plan.travel", { n: i.travel_min })}{conflicts.has(i.id) ? ` — ${t("plan.conflict")}` : ""}</div> : null}
                    <Link href={`${base}/plan/${i.id}`} className="card flex items-start gap-3 p-3.5">
                      {ph ? <img src={ph} alt="" className="thumb" /> : <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-surface-2 text-[1.375rem]">{i.emoji}</span>}
                      <div className="min-w-0 flex-1"><div className="font-bold">{itemTitle(i, locale)}</div><div className="text-[0.8125rem] text-ink-2">{pl ? `${placeName(pl, locale)}${pl.area ? ` · ${pl.area}` : ""}` : i.address || ""}{i.end_time && i.end_time !== i.start_time ? ` · ${fmtTime(i.end_time, locale)}` : ""}</div>{itemNote(i, locale) && <div className="mt-0.5 line-clamp-2 text-[0.8125rem] text-ink-2">{itemNote(i, locale)}</div>}
            ...
            ) : <div className="py-10 text-center text-ink-2"><div className="text-[2.5rem]">🗓️</div><b className="text-ink">{t("plan.noPlans", { day: dayLabel(b.trip, day, locale) })}</b><p>{t("plan.noPlansSub")}</p>{canEdit && <Link href={`${base}/plan/new?day=${day}`} className="btn btn-sun mt-4 px-8 py-4 text-[1rem]">{t("plan.addActivity")}</Link>}</div>}
          </div>
          <div className="hidden md:block"><div className="sticky top-6 card"><div className="eyebrow mb-2">{dayLabel(b.trip, day, locale)}</div>{info?.caption && <p className="text-[0.875rem] text-ink-2">{info.caption}</p>}<div className="mt-3 grid grid-cols-2 gap-2">{items.filter(i => photoOf(b, i)).slice(0, 6).map(i => <img key={i.id} src={photoOf(b, i)!} alt="" className="h-24 w-full rounded-xl object-cover" />)}</div></div></div>
        </div>
      )}
    </div>
  );
