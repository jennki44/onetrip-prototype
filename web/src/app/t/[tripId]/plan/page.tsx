      ...
      {view === "calendar" ? (
        <div className="grid grid-cols-3 gap-2.5">
          {Array.from({ length: total }, (_, i) => i + 1).map(d => { const it = itemsOnDay(b, d); const di = b.days.find(x => x.day === d); return (
            <Link key={d} href={`${base}/plan?day=${d}`} className={`card min-h-[120px] p-3 ${d === todayDay ? "outline outline-2 outline-coral" : ""}`}>
              <div className="font-display text-[15px] font-bold">Day {d}</div><div className="mb-2 text-[11.5px] text-ink-3">{dayLabel(b.trip, d, locale)}{di?.theme ? ` · ${di.theme}` : ""}</div>
        ...
        <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
          <div>
            ...
            {info?.banner_url && <div className="relative mt-3 overflow-hidden rounded-[28px]"><img src={info.banner_url} alt="" className="h-[170px] w-full object-cover" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[rgba(10,16,20,.82)] to-transparent px-4 pb-3.5 pt-10 text-white"><div className="eyebrow text-white/85">Day {day}{info.theme ? ` · ${info.theme}` : ""}</div><div className="font-display text-[18px] font-bold">{dayLabel(b.trip, day, locale, true)}</div><div className="text-[12.5px] text-white/85">{info.stay ? `${t("plan.stay")} ${info.stay}` : ""}{info.drive ? ` · ${t("plan.driving")} ${info.drive}` : ""}</div></div></div>}
