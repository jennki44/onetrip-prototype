      ...
      {view === "calendar" ? (
        ...
        <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
          <div>
            ...
            {info?.banner_url && <div className="relative mt-3 overflow-hidden rounded-[28px]"><img src={info.banner_url} alt="" className="h-[170px] w-full object-cover" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[rgba(10,16,20,.82)] to-transparent px-4 pb-3.5 pt-10 text-white"><div className="eyebrow text-white/85">{t("ui.dayN", { n: day })}{info.theme ? ` · ${info.theme}` : ""}</div><div className="font-display text-[1.125rem] font-bold">{dayLabel(b.trip, day, locale, true)}</div><div className="text-[0.7813rem] text-white/85">{info.stay ? `${t("plan.stay")} ${info.stay}` : ""}{info.drive ? ` · ${t("plan.driving")} ${info.drive}` : ""}</div></div></div>}
            ...
            ) : <div className="py-10 text-center text-ink-2"><div className="text-[2.5rem]">🗓️</div><b className="text-ink">{t("plan.noPlans", { day: dayLabel(b.trip, day, locale) })}</b><p>{t("plan.noPlansSub")}</p></div>}
          </div>
