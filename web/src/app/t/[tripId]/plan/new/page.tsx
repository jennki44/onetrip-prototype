...
export default async function NewActivity({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ day?: string; place?: string; edit?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[560px]"><Link href={edit ? `${base}/plan/${edit.id}` : `${base}/plan`} className="mb-2 inline-block text-[0.875rem] font-extrabold text-ink-2">‹ {t("common.back")}</Link><h1 className="mb-4 text-[1.75rem]">{edit ? t("common.edit") : t("plan.addActivity").replace("+ ", "")}</h1>
      <form action={saveActivity} className="flex flex-col gap-3">
        ...
        <Field label={t("ui.location")}><select name="placeId" defaultValue={i.place_id || ""} className="input"><option value="">{t("ui.choosePlace")}</option>{b.places.map(p => <option key={p.id} value={p.id}>{p.emoji} {(locale.startsWith("zh") && p.name_zh) || p.name}</option>)}</select></Field>
