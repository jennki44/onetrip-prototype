...
export default async function NewActivity({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ day?: string; place?: string; edit?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[560px]"><Link href={edit ? `${base}/plan/${edit.id}` : `${base}/plan`} className="btn btn-sm mb-3">‹ {t("common.back")}</Link><h1 className="mb-4 text-[1.75rem]">{edit ? t("ui.editActivity") : t("plan.addActivity").replace("+ ", "")}</h1>
      <form action={saveActivity} className="flex flex-col gap-3">
        ...
        <Field label={t("ui.activityName")}><input name="title" required maxLength={120} defaultValue={i.title} className="input" placeholder={t("ui.activityHint")} /></Field>
        <Field label={t("money.date")}><select name="day" defaultValue={i.day} className="input">{Array.from({ length: dayCount(b.trip) }, (_, k) => k + 1).map(d => <option key={d} value={d}>{dayLabel(b.trip, d, locale)}</option>)}</select></Field><div className="grid grid-cols-2 gap-2"><Field label={t("ui.start")}><input name="start" type="time" required defaultValue={i.start_time.slice(0, 5)} className="input" /></Field><Field label={t("ui.end")}><input name="end" type="time" defaultValue={(i.end_time || "").slice(0, 5)} className="input" /></Field></div>
        <Field label={t("ui.location")}><PlacePicker initial={i.place_id || ""} places={b.places.map(p => ({ id: p.id, label: `${p.emoji} ${(locale.startsWith("zh") && p.name_zh) || p.name}` }))} /></Field>
        <Field label={t("ui.people")}><div className="flex flex-wrap gap-1.5">{b.members.map(m => <label key={m.user_id} className="flex items-center gap-1.5 rounded-full border-2 border-line bg-surface py-1 pl-1 pr-3 text-[0.8125rem] font-extrabold has-[:checked]:border-teal has-[:checked]:bg-teal-soft"><input type="checkbox" name="people" value={m.user_id} defaultChecked={i.participant_ids.includes(m.user_id)} className="hidden" /><span className="avatar" style={{ background: m.profile.color, width: 22, height: 22, fontSize: 9 }}>{m.profile.initials}</span>{m.profile.name}</label>)}</div></Field>
