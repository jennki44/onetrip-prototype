...
export function TripMap({ tripId, places, todayIds, bookedIds, focus, initialFilter, links }: { tripId: string; places: Place[]; todayIds: string[]; bookedIds: string[]; focus: string | null; initialFilter: string; links: Links }) {
  ...
  return (
    <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
      <div>
        <div className="-mx-4 mb-3 flex gap-2 overflow-x-auto px-4 [scrollbar-width:none]">{FILTERS.map(k => <button key={k} onClick={() => { setFilter(k); setSel(null); }} className={`shrink-0 rounded-full border-2 px-3.5 py-1.5 text-[13px] font-extrabold ${filter === k ? "border-ink bg-ink text-ground" : "border-line bg-surface text-ink-2"}`}>{t(`ui.map.${k}`)}</button>)}</div>
      ...
      <div className="mt-3 md:mt-0">
        {p ? (
          <div className="card">{p.photo_url && <img src={p.photo_url} alt="" className="mb-3 h-[170px] w-full rounded-2xl object-cover" />}<div className="flex items-start justify-between"><div><div className="text-[20px] font-bold">{p.emoji} {nm(p)}</div><div className="text-ink-2">{p.rating ? `⭐ ${p.rating} · ` : ""}{p.from_hotel_min != null ? t("ui.map.fromStay", { n: p.from_hotel_min }) : p.area}{p.price_level ? ` · ${p.price_level}` : ""}</div>{p.hours && <div className="text-[12.5px] text-ink-3">{t("ui.map.open", { hours: p.hours })}</div>}</div><button onClick={() => setSel(null)} className="text-ink-3">✕</button></div>
            <div className="mt-3"><div className="eyebrow mb-1.5">{t("ui.map.inTrip")}</div><div className="flex flex-wrap gap-1.5">{links[p.id]?.items.map(i => <Link key={i.id} href={`${base}/plan/${i.id}`} className="pill pill-teal">📅 Day {i.day} · {i.title}</Link>)}{links[p.id]?.decisions.map(d => <Link key={d.id} href={`${base}/decisions/${d.id}`} className="pill pill-teal">🗳 {d.title}</Link>)}{links[p.id]?.expenses ? <Link href={`${base}/money?tab=expenses`} className="pill pill-teal">💰 {links[p.id].expenses}</Link> : null}{p.saved && <span className="pill pill-good">{t("ui.map.saved2")}</span>}{!links[p.id]?.items.length && !links[p.id]?.decisions.length && <span className="text-[12.5px] text-ink-3">{t("ui.map.notInPlan")}</span>}</div></div>
        ...
        ) : <div className="card divide-y divide-line-2 p-0">{shown.map(pl => <button key={pl.id} onClick={() => setSel(pl.id)} className="flex w-full items-center gap-3 px-4 py-3 text-left">{pl.photo_url ? <img src={pl.photo_url} alt="" className="h-10 w-10 rounded-xl object-cover" /> : <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-surface-2 text-[19px]">{pl.emoji}</span>}<div className="min-w-0 flex-1"><div className="truncate font-bold">{nm(pl)}</div><div className="text-[13px] text-ink-2">{pl.type} · {pl.area}{pl.rating ? ` · ⭐ ${pl.rating}` : ""}</div></div><span className="text-[12px] text-ink-3">{pl.from_hotel_min != null ? `${pl.from_hotel_min} min` : ""}</span></button>)}</div>}
      </div>
    </div>
  );
}
