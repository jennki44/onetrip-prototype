...
export function TripMap({ tripId, places, todayIds, bookedIds, focus, initialFilter, links }: { tripId: string; places: Place[]; todayIds: string[]; bookedIds: string[]; focus: string | null; initialFilter: string; links: Links }) {
  const { t } = useT(); const [filter, setFilter] = useState(initialFilter); const [sel, setSel] = useState<string | null>(focus);
  ...
  return (
    <div className="md:grid md:grid-cols-[1.15fr_.85fr] md:gap-6">
      ...
      <div className="mt-3 md:mt-0">
        {p ? (
          <div className="card">{p.photo_url && <img src={p.photo_url} alt="" className="mb-3 h-[170px] w-full rounded-2xl object-cover" />}<div className="flex items-start justify-between"><div><div className="text-[20px] font-bold">{p.emoji} {p.name}</div><div className="text-ink-2">{p.rating ? `⭐ ${p.rating} · ` : ""}{p.from_hotel_min != null ? t("ui.map.fromStay", { n: p.from_hotel_min }) : p.area}{p.price_level ? ` · ${p.price_level}` : ""}</div>{p.hours && <div className="text-[12.5px] text-ink-3">{t("ui.map.open", { hours: p.hours })}</div>}</div><button onClick={() => setSel(null)} className="text-ink-3">✕</button></div>
