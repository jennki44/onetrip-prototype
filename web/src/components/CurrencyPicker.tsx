...
export function CurrencyPicker({ tripId, current }: { tripId: string; current: string }) {
  ...
  return (
    <select value={current} disabled={pending} onChange={e => start(() => setReportingCurrency(tripId, e.target.value))} className="rounded-lg border-2 border-line bg-surface px-2 py-1.5 text-[13px] font-extrabold" aria-label="Reporting currency">
