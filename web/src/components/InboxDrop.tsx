...
export function InboxDrop({ tripId, forItem, bookings }: { tripId: string; forItem: { id: string; title: string; day: number } | null; bookings: { id: string; title: string }[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <label className="flex cursor-pointer flex-col items-center gap-1.5 rounded-[22px] border-2 border-dashed border-line bg-surface px-4 py-8 text-center"><span className="text-[36px]">📥</span><span className="font-bold">{file ? file.name : t("ui.inbox.types")}</span><span className="text-[12.5px] text-ink-3">{file ? `${Math.round(file.size / 1024)} KB` : t("ui.inbox.choose")}</span><input type="file" className="hidden" accept="image/jpeg,image/png,image/webp,image/heic,image/heif,application/pdf,.eml,.txt" onChange={e => setFile(e.target.files?.[0] || null)} /></label>
